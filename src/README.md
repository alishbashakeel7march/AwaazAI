# MultiModel — آواز · Voice for Everyone

A 100% free, voice-first MultiModel assistant for regional language speakers and low-literacy populations in Pakistan. Users **speak** in Urdu, Pashto, Sindhi, Punjabi, or Balochi, and MultiModel answers **out loud** — no typing, no reading, no sign-up.
...
- **TTS:** Answers are spoken using the Google Translate TTS proxy for Urdu / Microsoft Edge TTS fallback for Sindhi, Pashto, Punjabi, and Balochi. If the network backends fail, the browser's `speechSynthesis` is used as a last resort.
